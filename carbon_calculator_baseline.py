...
def parse_emails(filepath: str) -> list[dict]:
    ...
    system = """You are a data extraction assistant analysing exported email files.
For each individual email found, extract:
1. word_count - words in the body only (exclude headers like From/To/Sent/Subject, and quoted reply text). If there is no body text, use 0.
2. recipients - number of recipients (To + Cc combined, minimum 1)
...
def parse_documents(path: str) -> list[dict]:
    ...
    system = """You are a data extraction assistant.
                Count the total number of words in the body text of this document chunk.
                Exclude page numbers. Include headings, footers, captions, and reference lists if present.
...
audio_video_extensions = {".opus", ".mp4", ".mp3", ".mov", ".aac", ".m4a", ".ogg", ".wav"}
...
"""
    ...
    system = "You are a digital sustainability analyst specialising in reducing the carbon footprint of student project workflows. Provide clear, practical, evidence-based recommendations."
