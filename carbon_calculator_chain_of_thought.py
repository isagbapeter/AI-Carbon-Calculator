...
def parse_whatsapp(filepath: str) -> dict:
    ...
    if regex_count > 0:
        ...
        system = (
            ...
            Step 1: Identify what a WhatsApp message looks like in this export (timestamp + sender + message).
            Step 2: Count how many such messages appear in the sample.
            Step 3: RETURN YOUR FINAL ANSWER AS A JSON ARRAY WITH NO EXPLANATION:
...
def parse_emails(filepath: str) -> list[dict]:
    ...
    Follow these steps:
    Step 1: Identify how many individual emails are present by looking for header patterns (From/To/Sent/Subject).
    Step 2: For each email, locate the body text (this is everything after the last header line).
    Step 3: Count only the body words, excluding any quoted reply text (lines beginning with > or From:).
    Step 4: Count the recipients from the To and Cc fields (minimum 1).
    Step 5: RETURN YOUR FINAL ANSWER AS A JSON ARRAY WITH NO EXPLANATION:
    [{"email_index": N, "word_count": W, "recipients": R}]
...
def parse_documents(path: str) -> list[dict]:
    ...
    Follow these steps:
    Step 1: Identify what type of content is in this chunk (body text, headers, footers, references).
    Step 2: Count all words excluding page numbers. Include headings, footers, captions, and reference lists.
    Step 3: RETURN YOUR FINAL ANSWER AS A JSON ARRAY WITH NO EXPLANATION:
    {"word_count": <integer>}
"""
...
"""
    ...
    system = """
    You are a digital sustainability analyst. Follow these steps:

    Step 1: Identify the artefact types that contribute the most CO2e and explain why.
    Step 2: Identify any duplicate files and what they suggest about file management practices.
    Step 3: For each inefficiency identified, suggest specific actionable changes.
    Step 4: Write your final recommendations in plain prose with no bullet points or dashes.
