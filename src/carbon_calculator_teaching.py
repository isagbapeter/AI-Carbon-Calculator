...
def parse_whatsapp(filepath: str) -> dict:
    ...
    if regex_count > 0:
        ...
        system = (
            """
            You are a data extraction assistant. Here is an example of how to count WhatsApp messages:

            Input: "[01/02/2024, 09:15:32] [STUDENT3]: Good morning everyone
                    [01/02/2024, 09:17:45] [STUDENT1]: Morning!
                    [01/02/2024, 09:20:01] [STUDENT3]: What time is the meeting?"

            Output: {"message_count": 3}
            Now apply the same method to the following WhatsApp export and return ONLY: {"message_count": <integer>}
...
def parse_emails(filepath: str) -> list[dict]:
    ...
    system = """
    You are a data extraction assistant. Here is an example of how to count email words:

Input:
From: [STUDENT3]
To: [STUDENT2], [STUDENT1]
Sent: 01 Jan 2024
Subject: Project update

Hi [STUDENT2] and [STUDENT1], please review the attached document and let me know your thoughts by Friday.
...
> Hi [STUDENT2] and [STUDENT1], please review the attached document and let me know your thoughts by Friday.
Thanks [STUDENT3], will do.

Output: [{"email_index": 1, "word_count": 18, "recipients": 2}, {"email_index": 2, "word_count": 4, "recipients": 2}]

Note: email 1 body has 18 words. Email 2 body has only "Thanks [STUDENT3], will do" = 4 words, excluding the quoted reply.
Always return at least one entry per file even if body is empty.
YOU MUST RETURN ONLY VALID JSON with no explanation or markdown.
Now apply this to the following email thread:
"""
...
def parse_documents(path: str) -> list[dict]:
    ...
    system = """
    You are a data extraction assistant. Here is an example of how to count document words:

Input chunk:

"Introduction
This paper explores the carbon footprint of digital student projects. We analyse five artefact types.
...
Output: {"word_count": 14}
Note: Exclude page numbers. Include all other content including headings, footers, captions, and reference lists.

Now apply this to the following document chunk and return ONLY: {"word_count": <integer>}
"""
...
"""
    ...
    system = """
    You are a digital sustainability analyst. Here is an example of a good recommendation:

    "The project generated 805 WhatsApp messages contributing 62% of total emissions. Future projects should consolidate daily communication into scheduled structured meetings to reduce message volume."
    
    Now apply this same level of specificity to the following CO2e data and provide recommendations in plain prose with no bullet points or dashes but organized into paragraphs according to each artefact.
"""
